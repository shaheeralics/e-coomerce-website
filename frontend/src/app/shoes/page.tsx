...
// Wrapper component to satisfy Suspense requirements in Next.js for useSearchParams
...
function ShoesCatalogContent() {
  ...
  useEffect(() => {
    ...
    if (genderParam) {
      setSelectedGenders([genderParam.toLowerCase()]);
    } else {
      setSelectedGenders([]);
    }
  ...
  const processedShoes = useMemo(() => {
    ...
    // 1. Gender Filter (maps to categories: men, women or shoe names/specs)
  ...
  const activeFiltersCount = selectedGenders.length + selectedCategories.length + selectedSizes.length + selectedColors.length + (maxPrice < 300 ? 1 : 0);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 flex-grow w-full flex flex-col">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <span className="text-[9px] font-bold uppercase tracking-widest text-neutral-400">All Products</span>
          <h1 className="text-2xl sm:text-3xl font-black uppercase tracking-widest text-neutral-900 mt-1">
            Velocity Shoes ({processedShoes.length})
          </h1>
        </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center flex-grow border border-dashed border-neutral-200 p-8">
              ...
              <p className="text-xs text-neutral-500 max-w-sm mb-6 uppercase tracking-wider">
                Try adjustment of filters, sizes, or search categories to discover what you are looking for.
              </p>
              <button
                onClick={resetFilters}
