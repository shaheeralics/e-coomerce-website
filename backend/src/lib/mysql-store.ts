...
let inMemoryCategorySettings = [
  { category_name: 'Men', is_visible: true, sort_order: 0 },
  { category_name: 'Women', is_visible: true, sort_order: 1 },
  { category_name: 'Kids', is_visible: true, sort_order: 2 },
  { category_name: 'Running', is_visible: true, sort_order: 3 },
  { category_name: 'Casual', is_visible: true, sort_order: 4 },
  { category_name: 'Limited Drops', is_visible: true, sort_order: 5 }
];
...
export async function getDbCategorySettings(): Promise<{ category_name: string; is_visible: boolean; sort_order: number }[]> {
  const isOnline = await testConnection();
  if (!isOnline) {
    return inMemoryCategorySettings.sort((a, b) => a.sort_order - b.sort_order);
  }
  try {
    const rows = await query<any[]>('SELECT category_name, is_visible, sort_order FROM category_settings ORDER BY sort_order ASC, category_name ASC');
    return rows.map(r => ({
...
export async function updateDbCategorySortOrders(orders: { categoryName: string; sortOrder: number }[]): Promise<boolean> {
  const isOnline = await testConnection();
  if (!isOnline) {
    orders.forEach(o => {
      const setting = inMemoryCategorySettings.find(s => s.category_name.toLowerCase() === o.categoryName.toLowerCase());
      if (setting) {
        setting.sort_order = o.sortOrder;
      }
    });
    return true;
  }
  try {
    for (const o of orders) {
      await query('UPDATE category_settings SET sort_order = ? WHERE category_name = ?', [o.sortOrder, o.categoryName]);
    }
    return true;
  } catch (error) {
    console.error('[MySQL Store] updateDbCategorySortOrders error:', error);
    return false;
  }
}
...
export async function getDbCustomPages(): Promise<CustomPage[]> {
  const isOnline = await testConnection();
  if (!isOnline) {
    return inMemoryCustomPages.sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0));
  }
  try {
    const rows = await query<any[]>('SELECT * FROM custom_pages ORDER BY sort_order ASC, created_at DESC');
    return rows.map(r => ({
      id: r.id,
      title: r.title,
      slug: r.slug,
...
export async function saveDbCustomPage(page: CustomPage): Promise<boolean> {
  ...
  try {
    ...
    if (rows.length > 0) {
      await query(
        'UPDATE custom_pages SET title = ?, slug = ?, content = ?, is_published = ?, sort_order = ? WHERE id = ?',
        [page.title, page.slug, page.content, page.isPublished ? 1 : 0, page.sortOrder || 0, page.id]
      );
    } else {
      await query(
        'INSERT INTO custom_pages (id, title, slug, content, is_published, sort_order) VALUES (?, ?, ?, ?, ?, ?)',
        [page.id, page.title, page.slug, page.content, page.isPublished ? 1 : 0, page.sortOrder || 0]
      );
    }
    return true;
  } catch (error) {
...
export async function updateDbCustomPageSortOrders(orders: { id: string; sortOrder: number }[]): Promise<boolean> {
  const isOnline = await testConnection();
  if (!isOnline) {
    orders.forEach(o => {
      const p = inMemoryCustomPages.find(page => page.id === o.id);
      if (p) {
  ...
  try {
    for (const o of orders) {
      await query('UPDATE custom_pages SET sort_order = ? WHERE id = ?', [o.sortOrder, o.id]);
    }
    return true;
  } catch (error) {
    console.error('[MySQL Store] updateDbCustomPageSortOrders error:', error);
    return false;
  }
}
