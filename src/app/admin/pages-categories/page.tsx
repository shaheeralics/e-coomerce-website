...
import { 
  ...
  reorderCustomPagesAction
} from '@/lib/actions';
import { CustomPage } from '@/types';
import { ToggleLeft, ToggleRight, Plus, Trash2, Edit2, X, Save, Loader2, ClipboardList, CheckSquare, ArrowUp, ArrowDown } from 'lucide-react';

export default function AdminPagesCategoriesPage() {
  const [categories, setCategories] = useState<{ category_name: string; is_visible: boolean; sort_order: number }[]>([]);
  ...
  const moveCategory = async (index: number, direction: 'up' | 'down') => {
    ...
    try {
      const payload = updatedCategories.map((c) => ({
        categoryName: c.category_name,
        sortOrder: c.sort_order
      }));
      await reorderCategoriesAction(payload);
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const movePage = async (index: number, direction: 'up' | 'down') => {
    ...
    try {
      const payload = updatedPages.map((p) => ({
        id: p.id,
        sortOrder: p.sortOrder || 0
      }));
      await reorderCustomPagesAction(payload);
    } catch (err) {
      console.error(err);
    }
  };
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col gap-1">
                      <button
                        ...
                        disabled={idx === 0}
                        className="p-1 bg-neutral-900 hover:bg-neutral-800 border border-neutral-800 disabled:opacity-30 disabled:hover:bg-transparent text-neutral-400 hover:text-white rounded-xs transition-colors cursor-pointer"
                        aria-label="Move Up"
                      >
